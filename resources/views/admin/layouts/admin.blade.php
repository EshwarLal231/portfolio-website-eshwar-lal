...
<html lang="en">
    ...
    <div class="container-scroller">
      <nav class="navbar default-layout-navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
        ...
        <div class="navbar-menu-wrapper d-flex align-items-stretch">
          ...
          <form class="d-flex align-items-center h-100" id="search-form" action="{{ route('admin.portfolio.search') }}" method="GET">
              <div class="input-group">
                <div class="input-group-prepend bg-transparent">
                  <i class="input-group-text border-0 mdi mdi-magnify"></i>
                </div>
                <input type="text" class="form-control bg-transparent border-0" id="search-input" name="search" placeholder="Search projects" required>
              </div>
            </form>
          </div>
  ...
  <script>
      $(document).ready(function(){
        // Capture the input event from the search field
        $('#search-input').on('keyup', function(){
          var searchQuery = $(this).val(); // Get the value entered in the search bar

          if(searchQuery.length > 2) { // Minimum 3 characters for search
            $.ajax({
              url: "{{ route('admin.portfolio.search') }}", // Route to search
              method: "GET",
              data: {
                search: searchQuery
              },
              success: function(response) {
                ...
                $('#search-results').html(response); // Assuming the backend returns HTML content
              }
            });
          } else {
